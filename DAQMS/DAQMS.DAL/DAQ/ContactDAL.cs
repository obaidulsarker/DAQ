    ...
    public class ContactDAL : DataAccessBase<ContactViewModel>
    {
        ...
        public override System.Int32 Save(ContactViewModel item, string mood)
        {
            ...
            try
            {
                ...
                command.Parameters.Add(new NpgsqlParameter("is_create_user", NpgsqlDbType.Bit));
        ...
        {
            return GetObjList(item.Id, item.CompanyId, item.ProjectId, item.ProjectName, item.ContactID, item.ContactName, item.LoginUserID, 0, 1).FirstOrDefault();
        }
    }
}
