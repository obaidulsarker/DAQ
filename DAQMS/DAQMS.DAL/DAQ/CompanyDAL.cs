...
namespace DAQMS.DAL
{
    public class CompanyDAL : DataAccessBase<CompanyViewModel>
    {
        ...
        public override System.Int32 Save(CompanyViewModel item, string mood)
        {
            int id = 0;
            ...
            try
            {
                ...
                NpgsqlCommand command = new NpgsqlCommand("save_company", conn);
                command.CommandType = CommandType.StoredProcedure;

                command.Parameters.Add(new NpgsqlParameter("p_id", NpgsqlDbType.Integer));
                command.Parameters[0].Value = item.Id;

                command.Parameters.Add(new NpgsqlParameter("p_company_name", NpgsqlDbType.Varchar));
                command.Parameters[1].Value = item.CompanyName;

                command.Parameters.Add(new NpgsqlParameter("p_phone", NpgsqlDbType.Varchar));
                command.Parameters[2].Value = item.Phone;

                command.Parameters.Add(new NpgsqlParameter("p_fax", NpgsqlDbType.Varchar));
                command.Parameters[3].Value = item.Fax;
        ...
        public List<CompanyViewModel> GetObjList(int id, string companyName, string phone, string email, string loginUserId, int startRowIndex, int maximumRows)
        {
            DataSet dsResult = new DataSet();
            DataTable dt = new DataTable();
            NpgsqlConnection conn = null;

            List<CompanyViewModel> results = null;
            ...
            try
            {
                ...
                NpgsqlCommand command = new NpgsqlCommand("get_company", conn);
                command.CommandType = CommandType.StoredProcedure;
                ...
                command.Parameters.Add(new NpgsqlParameter("p_company_name", NpgsqlDbType.Varchar));
                command.Parameters[1].Value = companyName;

                command.Parameters.Add(new NpgsqlParameter("p_phone", NpgsqlDbType.Varchar));
                command.Parameters[2].Value = phone ;
                ...
                // Fetch rows 
                results = new List<CompanyViewModel>();
        ...
        public override CompanyViewModel GetObjById(int id)
        {
            return GetObjList(id, "","","", "", 1, 1).FirstOrDefault();
        }

        public override List<CompanyViewModel> GetObjList(CompanyViewModel item, int startRowIndex, int maxRow)
        {
            return GetObjList(item.Id, item.CompanyName,item.Phone,item.Email, item.LoginUserID, startRowIndex, maxRow);
        }

        public override CompanyViewModel GetObjList(CompanyViewModel item)
        {
            return GetObjList(item.Id, item.CompanyName, item.Phone, item.Email, item.LoginUserID, 0, 1).FirstOrDefault();
        }
    }
}
